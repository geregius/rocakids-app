...
- **Bloqueo de niño sin documento (2026-08-14):** pensando en la migración del Módulo 2 (muchos de los ~578 niños reales no tienen `identificacionMenor`), un niño YA registrado **sin número de documento no puede tener ningún movimiento** (ni Entrada ni Salida) — la pantalla oculta el formulario y muestra el motivo en rojo, con sugerencia de avisarle a un admin (solo un admin puede completar ese dato, ver edición de `ninos` arriba). Esto se aplica en 3 capas: la UI lo oculta, `_registrarMovimientoNino()` lo revalida antes de enviar, y **la regla de Firestore también lo exige** (`get()` sobre `ninos/{fkIdNino}.identificacionMenor != ''`) — así que no se puede saltar aunque alguien modifique la app. Los niños VISITANTE no aplican (nunca tienen documento por diseño).
...
| `registrar_familia_screen.dart` | Sección "Registrar familia" — **solo Maestro Principal/Auxiliar** (2026-08-14). Mismo formulario que el auto-registro público de acudiente (acudiente + primer niño + relación), pero lo llena el maestro EN NOMBRE de la familia (ej. en la mesa de registro de un servicio). Al terminar, puede registrar otra familia sin salir de su propia sesión. Ver `AuthService.registrarAcudienteConNinoDesdeServidor()` — usa una app de Firebase secundaria y temporal para crear la cuenta nueva, así el maestro nunca pierde su propia sesión (`createUserWithEmailAndPassword` normalmente dejaría logueado como la cuenta recién creada). No requirió cambios en `firestore.rules`/`storage.rules`: los escritos ocurren autenticados como el nuevo acudiente, igual que el auto-registro. |
...
- `ninos`: create-only para cualquier autenticado. `update` para admin, o para el padre/madre vinculado (`esPadreOMadreDe()`, ver función arriba) — excepto documento/estado/foto, que quedan admin-only.
