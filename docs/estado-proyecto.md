...
7. **Consolidar despliegues — desplegar UNA sola vez, no varias seguidas.** Cada deploy de una Cloud Function Gen2 deja basura en **DOS** lugares, no uno:
   - **Artifact Registry** (`gcf-artifacts`) — la imagen del contenedor. Nivel gratuito **500 MB/proyecto**. **Verificado 2026-09-06:** este proyecto SÍ tiene limpieza automática (`firebase-functions-cleanup`, borra todo lo de más de 24 h, en las dos regiones) y los repos reportan 0 MB. **Ya pasó de verdad en otro proyecto:** 62 despliegues en 2 días llevaron a `sigil3` a 883 MB (176% del tope).
   - **`gcf-v2-sources-<proyecto>-<región>`** — copia del código fuente. **En este proyecto tampoco se acumula, pero por un mecanismo distinto** (verificado 2026-09-06): los dos buckets tienen **versionado activado** y una regla de ciclo de vida `{isLive: false, numNewerVersions: 3}`. Firebase escribe siempre en la MISMA ruta por función (`<NombreFuncion>/function-source.zip`), así que cada deploy convierte la copia anterior en una versión archivada y la regla conserva solo las 3 últimas. Tamaño real: 168 KiB (São Paulo) y 197 KiB (us-central1), acotado por diseño a ~9 funciones × 4 versiones × 44 KB ≈ 1,6 MB.

   ⚠️ **NO aplicar acá la regla genérica `{"action":{"type":"Delete"},"condition":{"age":3}}`** que circula para este caso. Sin `isLive: false`, esa condición alcanza también a los objetos VIVOS — borraría el `function-source.zip` actual de toda función que lleve más de 3 días sin redesplegarse, que es el estado normal de casi todas. La regla que ya está puesta es la correcta para este layout; no hace falta tocar nada.
...
## 5.17. "Programación de Servidores" — Etapa 1: grupos/equipos (2026-08-31)

Pedido de Rafael: el ministerio organiza a los servidores en equipos que rotan por categoría — **Domingos** (2 grupos hoy), **Miércoles** (6), **Casa2** (3), **Ayunos** (5). Primera etapa, a propósito acotada: solo crear/editar/eliminar grupos y asignarles integrantes — la programación de TURNOS en sí (quién sirve qué día puntual) queda para una siguiente etapa, todavía no diseñada.

**Permiso — a propósito MÁS ACOTADO que el resto de pantallas de liderazgo:** Rafael pidió explícitamente "solo los líderes del ministerio". A diferencia de "Gestión de Servidores"/"Acudientes y Niños"/Dashboard (administrador + columna + líder de ministerio), acá **columna NO tiene acceso** — ni para ver ni para editar. `RolUsuario.puedeGestionarProgramacion` (nuevo getter, `usuario_app.dart`) y `puedeGestionarProgramacion()` (nueva función, `firestore.rules`) deben quedar sincronizados si esto cambia.

**Modelo de datos — dos colecciones nuevas:**
- `categorias_programacion`: `nombre` (texto libre) + `creadoEn`. **Empezaron como una lista fija en el código** (`Domingos`, `Miércoles`, `Casa2`, `Ayunos`) — el mismo día, tras ver la pantalla, Rafael pidió poder crear categorías nuevas también (ej. "Retiro", "Campamento"), así que pasaron a esta colección propia, igual de administrable que los grupos. Las 4 categorías originales se sembraron una sola vez con un script REST (mismo patrón de `gcloud auth print-access-token` de migraciones anteriores) para que los grupos ya creados con esos nombres no se quedaran huérfanos.
- `grupos_servidores`: `categoria` (texto libre — el `nombre` de una `categorias_programacion`, guardado como string plano, no como referencia/ID) — **distinto** de `serviciosDisponibles` (`models/registro.dart`, los servicios donde se registra la asistencia de NIÑOS) aunque los nombres se parezcan; no se reutilizó esa lista a propósito, son conceptos separados (equipos de servidores vs. check-in de niños). `nombre`: texto libre (ej. "Grupo 1"). `fkIdsServidores`: array de uids de `usuarios` — un servidor puede estar en varios grupos a la vez, sin restricción de exclusividad. `creadoEn`: timestamp del servidor.

**`services/auth_service.dart`:** `listarCategoriasProgramacion()`/`crearCategoriaProgramacion()` + `listarGruposServidores()`/`crearGrupoServidores()`/`editarGrupoServidores()`/`eliminarGrupoServidores()` — todo streams de colecciones chicas, agrupar/ordenar en memoria del lado del cliente para no necesitar índices compuestos.

**`admin/programacion_servidores_screen.dart`:** botón "Nueva categoría" arriba de todo (diálogo simple con un `TextField`), luego una sección por cada categoría existente (con un botón "+" propio para crear un grupo ya con esa categoría preseleccionada), cada grupo como tarjeta (nombre + cuántos/quiénes lo integran) que abre una hoja inferior de edición. Formulario de crear/editar (`_GrupoServidoresSheet`): categoría (dropdown con TODAS las categorías existentes + una opción final "+ Nueva categoría..." que abre el mismo diálogo sin salir del formulario), nombre (texto), y selector múltiple de servidores activos (`AuthService.obtenerTodosLosServidoresActivos()`, la misma consulta ya usada para [[feature-cumpleanos-servidores-ordenado-proximidad]]) con buscador propio por nombre, checkboxes. Botón "Eliminar grupo" solo en modo edición, con el mismo diálogo de confirmación (`confirmarEliminar`) que niño/acudiente/servidor.

**`firestore.rules`:** las dos colecciones nuevas (`categorias_programacion`, `grupos_servidores`), `get`/`list`/`create`/`update`/`delete` todos exigen `puedeGestionarProgramacion()` — **cerradas por completo a cualquier otro rol, incluida la lectura** (ni siquiera un servidor puede ver en qué grupo está, por ahora — pendiente decidir en una etapa futura si conviene abrir lectura de "mis propios grupos" a cada servidor). `create` de `grupos_servidores` ya NO valida `categoria` contra una lista fija (dejó de tener sentido al volverse dinámica) — solo que sea texto no vacío.

Ítem de menú nuevo en `AppShell` ("Programación de Servidores", ícono `Icons.groups` — ya confirmado visualmente en otra pantalla, ver la lección sobre íconos nunca antes usados en sección 8), visible solo con `puedeGestionarProgramacion`.

---

## 5.18. "Programación de Servidores" — Etapa 2: rotación automática (2026-09-02)

Pedido de Rafael, mismo día que pidió las categorías dinámicas: **"el sistema debe asignar por defecto... la rotación correspondiente en el orden de cada grupo"**. Explicó el patrón real de cada categoría:
- **Domingos** y **Miércoles**: caen en un día fijo cada semana, un grupo distinto cada vez, en orden — rotación 100% automática.
- **Casa2** (un viernes al mes) y **Ayunos** (una vez al mes): NO caen en un día fijo — se programa la fecha exacta a mano cada vez, pero el sistema sugiere el siguiente grupo en el orden de rotación.
- **Enseñanza** (cada lunes) — categoría nueva que Rafael ya había creado él mismo antes de este ajuste (el grupo "Enseñanzas" que había quedado mal puesto bajo "Domingos" en la etapa 1, él mismo lo borró).

**3 preguntas confirmadas con Rafael antes de construir** (`AskUserQuestion`, no asumidas):
1. El punto de partida de la rotación semanal (qué grupo sirve el próximo domingo/miércoles) **lo fija Rafael una vez por categoría** — de ahí en adelante el sistema sigue solo. NO arranca automáticamente por "Grupo 1".
2. Para Casa2/Ayunos, al programar la fecha del mes, el sistema **sugiere** el siguiente grupo en el orden (por cuántas veces ha servido cada uno, no por semana calendario) — Rafael confirma o cambia antes de guardar.
3. La pantalla de "validar quién sirve" muestra **solo el próximo servicio** de cada categoría (no varias semanas por adelantado) — más simple para empezar.

**Modelo de datos:**
- `categorias_programacion` ganó 4 campos: `tipoRotacion` (`'semanal'` o `'manual'`, fijado al crear la categoría — ya no se puede cambiar después desde la UI), `diaSemana` (1=lunes..7=domingo, solo si es semanal), `fechaReferenciaRotacion` + `grupoReferenciaId` (el punto de partida: "el [fecha] sirve [grupo]", se fija/corrige aparte con `AuthService.configurarRotacionCategoria()`). Las 4 categorías originales + "Enseñanza" se re-clasificaron con un script REST (Domingos/Miércoles/Enseñanza → semanal con su día; Casa2/Ayunos → manual).
- `grupos_servidores` ganó `orden` (posición en la rotación, 0 = primero) — por defecto al final (según cuántos grupos ya tenía la categoría), con botones ↑/↓ en cada tarjeta (`AuthService.intercambiarOrdenGrupos()`, un `batch` que intercambia el `orden` de dos grupos) para corregirlo a mano. Los 13 grupos ya creados se re-numeraron con un script REST según su orden real de creación (que ya coincidía con "Grupo 1", "Grupo 2"...).
- Colección nueva `servicios_programados`: `categoria`, `fecha`, `grupoId`, `creadoEn` — una fila por cada ocasión programada a mano de una categoría `manual`.

**Cálculo de la rotación (`models/grupo_servidores.dart`, funciones puras, sin estado guardado que haya que ir avanzando):**
- `grupoQueSirve()`: para categorías semanales — cuenta cuántas semanas pasaron entre la fecha de referencia y la fecha objetivo, avanza esa cantidad de posiciones en la lista de grupos (ya ordenada por `orden`), en círculo (`%`). Determinístico — no depende de ningún Cloud Function ni de que nadie "avance" nada semana a semana.
- `siguienteGrupoSugerido()`: para categorías manuales — mira el `ServicioProgramado` más reciente de esa categoría y devuelve el siguiente grupo en el orden (o el primero si nunca se ha programado ninguno).
- `proximaFechaDia()`: próxima fecha (incluyendo hoy) que cae en un día de la semana dado, para saber "cuándo es el próximo domingo/miércoles/lunes".

**`admin/programacion_servidores_screen.dart` ahora con 2 pestañas** (antes solo tenía la lista de grupos):
- **"Grupos"** (la de la etapa 1, con los ajustes de arriba: botones ↑/↓, y el diálogo "Nueva categoría" ahora también pregunta si es semanal+día o manual). **2026-09-02, mismo día:** cada categoría es ahora una `ExpansionTile` colapsable (antes todas quedaban siempre abiertas) — arranca expandida (`initiallyExpanded: true`, a diferencia de la pestaña "Niños" de "Acudientes y Niños" que arranca colapsada — acá no se pidió eso).
- **"Próximos Servicios"** (nueva): una tarjeta por categoría. Semanal → fecha del próximo día + grupo calculado (o aviso de "todavía no se configuró el punto de partida" con botón para hacerlo, `_ConfigurarRotacionSheet` — el selector de fecha usa `selectableDayPredicate` para solo dejar elegir fechas del día de la semana correcto, evita configurar un punto de partida sin sentido). Manual → fecha+grupo del próximo `ServicioProgramado` (o "sin programar" con botón "Programar servicio", `_ProgramarServicioSheet`, que pre-llena el grupo sugerido) + botón "Quitar" por si hubo un error.

**`firestore.rules`:** nueva colección `servicios_programados`, mismo permiso que las otras dos (`puedeGestionarProgramacion()`, cerrada a cualquier otro rol incluida lectura). `categorias_programacion.create` ahora exige `tipoRotacion` en `['semanal', 'manual']`; `grupos_servidores.create` exige que `orden` sea un entero. Los campos que se fijan después vía `update` (`fechaReferenciaRotacion`/`grupoReferenciaId`) no tienen validación de formato en la regla — mismo patrón que el resto del archivo, `update` solo valida QUIÉN, no QUÉ (ver nota de la sección de validación de formato, más arriba en el archivo).

⚠️ **Nota técnica para la próxima vez que se use un spread `[...]` sobre un `Stream<List<T>>?.data`:** `flutter analyze` no detectó un error de tipo real (`List<dynamic>` en vez de `List<GrupoServidores>`) que sí hizo fallar `flutter build web` — pasó con `[...(snapshot.data ?? [])]..sort(...)`, donde el `?? []` (lista vacía sin tipo) dentro del spread confundía la inferencia de tipos de dart2js aunque el analizador lo aceptara. Se corrigió anotando el tipo explícito: `<GrupoServidores>[...]`. Si vuelve a pasar un caso raro donde `flutter analyze` da 0 issues pero `flutter build web` sí falla, sospechar primero de un spread sobre un `?? []` sin tipo explícito.

---

## 5.19. Notificaciones push — infraestructura + prueba (2026-09-02)

Idea de Rafael: cuando se asigne un grupo en "Programación de Servidores", que la app le mande una notificación real (de las que hacen sonar el celular) a cada servidor de ese grupo. **Primera etapa, a propósito acotada** (confirmado explícitamente): solo la infraestructura — botón para activar + un envío de prueba de punta a punta — **todavía NO está conectada a ningún evento real de la app** (ni a "asignar grupo" en Programación de Servidores, ni a nada más). Conectarla es el siguiente paso, pendiente de que Rafael la pida.

**Cómo funciona de verdad (explicado y confirmado con Rafael antes de construir):**
- El permiso de notificaciones NO se puede activar a control remoto — un navegador solo lo pide cuando la propia persona toca algo dentro de la página, nunca solo. Por eso es un botón, no algo automático.
- Solo funciona si la persona tiene la app instalada/agregada a su pantalla de inicio (en iPhone, solo si la agregó con Safari — ver limitaciones ya documentadas en [[feature-icono-pantalla-inicio-y-escaner-timeout]]) Y acepta el permiso.
- Si alguien ya lo había RECHAZADO antes, la app ya no se lo puede volver a pedir — tiene que entrar a la configuración del navegador/celular a mano.

**Costo: $0.** Firebase Cloud Messaging (FCM) no cobra por notificación enviada, sin importar el volumen — ver [[feedback-revision-costos-permanente]]. Lo único nuevo con algún costo teórico es la Cloud Function `enviarNotificacionPrueba` (invocaciones + lecturas de Firestore por envío), muy por debajo del nivel gratuito al volumen de este proyecto — mismo criterio que el resto de funciones ya desplegadas.

**Piezas nuevas:**
- **Llave VAPID** (pública, no secreta — mismo criterio que el resto de `firebase_options.dart`): generada por Claude directamente en Firebase Console (`console.firebase.google.com/.../settings/cloudmessaging` → "Certificados push web" → "Generate key pair") usando `claude-in-chrome` con la sesión ya iniciada de Rafael — sin necesidad de que él hiciera el clic. Guardada en `lib/services/notificaciones_service.dart`.
- **`web/firebase-messaging-sw.js`** (nuevo): service worker que recibe la notificación cuando la pestaña está en segundo plano o cerrada — usa los mismos valores de `firebase_options.dart` (bloque `web`). Agregado a la lista de `no-cache` en `firebase.json`, mismo criterio que el resto de archivos de caché crítica (sección 8).
- **`lib/services/notificaciones_service.dart`** (nuevo): `estadoPermiso()`, `activar(uid)` (pide el permiso, guarda el token del dispositivo en `usuarios/{uid}.fcmTokens` — un array, una persona puede tener varios dispositivos), `escucharRenovacionToken(uid)` (Firebase renueva el token de vez en cuando, hay que volver a guardarlo o ese dispositivo deja de recibir en silencio), `enviarNotificacionDePrueba()` (llama a la Cloud Function por HTTP directo — **NO con el paquete `cloud_functions`**, tiene el mismo bug conocido en Flutter Web que ya se documentó para `enviarCorreoRecuperacion`/`resetPassword` — a diferencia de esa función, esta SÍ exige sesión iniciada, así que hay que mandar el ID token a mano en `Authorization: Bearer`, la llamada HTTP directa no lo agrega sola).
- **`widgets/boton_notificaciones.dart`** (nuevo): la campana en la barra superior, junto a "Actualizar" (`widgets/app_shell.dart`, visible para cualquier usuario logueado). Campana rellena = activadas en este dispositivo; vacía = todavía no. Al activarse con éxito, dispara de inmediato `enviarNotificacionDePrueba()` — así la propia notificación de prueba sirve de confirmación real, no solo un aviso dentro de la app. ⚠️ `Icons.notifications`/`Icons.notifications_none` son íconos NUNCA antes usados en esta app — ver la lección de tree-shaking en sección 8, pendiente que Rafael confirme que sí se ven en producción.
- **`functions/index.js`**: `enviarNotificacionAUsuarios(uids, {...})` (helper interno, usa `admin.messaging().sendEachForMulticast()`, y limpia automáticamente cualquier token que Firebase devuelva como inválido/vencido — sin esto, un token muerto se seguiría intentando para siempre) + `enviarNotificacionPrueba` (`onCall`, sin restricción de rol — cualquiera logueado puede pedir que se le mande una notificación a SÍ MISMO, cero riesgo de privilegios). Región `us-central1` (por defecto, igual que `enviarCorreoRecuperacion`).
- **`firebase.json`:** `script-src` de la CSP (todavía en `Report-Only`, sección 8) ampliado con `https://www.gstatic.com` — el service worker de mensajería carga el SDK de Firebase desde ahí (`importScripts`), y cuando se active el modo forzado de CSP más adelante lo habría bloqueado si no se agregaba ahora.
- **Sin cambios en `firestore.rules`:** la regla de auto-edición de `usuarios/{uid}` ya permitía cualquier campo nuevo (como `fcmTokens`) salvo la lista específica de campos bloqueados (`rol`/`activo`/`correo`/`fechaVerificacionAntecedentes`), así que `arrayUnion` sobre `fcmTokens` ya estaba cubierto sin tocar nada.
- **`main.dart`:** `FirebaseMessaging.onMessage.listen(...)` + un `scaffoldMessengerKey` global — si una notificación llega mientras la pestaña está ABIERTA y activa, el navegador NO la muestra como notificación real del sistema (eso solo pasa en segundo plano/cerrada, vía el service worker); en primer plano se muestra como un `SnackBar` dentro de la app en su lugar. **Importante para cuando Rafael pruebe:** para ver la notificación de verdad en la bandeja del sistema, hay que minimizar/cambiar de pestaña después de activar, no quedarse mirando la app.

⚠️ **Riesgo técnico sin confirmar en dispositivo real todavía:** Flutter Web ya registra su propio service worker (`flutter_service_worker.js`, para caché de assets) en el mismo scope raíz (`/`) que `firebase-messaging-sw.js` necesita. En teoría dos service workers pueden coexistir con scopes que se solapan, pero es un punto de integración conocido como delicado en proyectos Flutter+FCM — no se pudo probar de punta a punta sin una cuenta logueada real (Rafael prueba la UI, no Claude). Si las notificaciones en segundo plano no llegan pero las de primer plano (`SnackBar`) sí, sospechar de esto primero.

**✅ Primer reporte real de Rafael (mismo día): "toco la campana en el iPhone pero no me sale nada" — la app estaba instalada desde Chrome.** Confirma la limitación YA documentada de este proyecto ([[feature-icono-pantalla-inicio-y-escaner-timeout]]): Apple solo permite Service Workers/notificaciones reales dentro de Safari — Chrome en iOS nunca soporta PWA de verdad, sin importar qué tan bien esté hecho el código. **Bug real encontrado en el camino:** `_BotonNotificacionesState._tocar()` no tenía `try/catch` alrededor de `activar()` — en un navegador sin soporte real (como este caso), el paquete `firebase_messaging` lanza una excepción en vez de devolver `false`, así que la campana literalmente "no hacía nada" en vez de mostrar un error explicando el problema. Corregido: `try/catch` que muestra "Este navegador no admite notificaciones. En iPhone, agrega RocaKids a la pantalla de inicio usando Safari (no Chrome)..." — y lo mismo protegido en `initState()`.

**✅ Segunda vuelta, mismo día: Rafael reinstaló con Safari (ícono en pantalla de inicio, iOS 17) y seguía viendo el MISMO mensaje** — pero resultó ser una respuesta CACHEADA de la versión vieja de la app, no el error real (síntoma clásico de este proyecto, ver el punto de `main.dart.js`/service worker más arriba en esta sección). El mensaje genérico "usa Safari" tampoco habría sido la causa correcta esta vez, así que se cambió el `catch` para mostrar el TEXTO REAL de la excepción en vez de un mensaje adivinado (`'No se pudo activar: $e'`) — con eso, tras cerrar la app del todo y reabrirla, salió el error real: `MissingPluginException: No implementation found for method Messaging#...`. Era el mismo patrón de "plugin nuevo sin `flutter clean`" de siempre (ver el punto de `image_picker` más abajo en esta sección) — corregido con `flutter clean` + rebuild + redeploy. **Pendiente:** que Rafael confirme que ahora sí funciona.

**Lección general de este ida y vuelta:** cuando un mensaje de error puede tener MÁS DE UNA causa posible (acá: navegador sin soporte vs. plugin mal registrado vs. caché vieja), mostrar el texto real del error desde el principio en vez de adivinar una sola causa y armar un mensaje fijo para esa — ahorra rondas completas de ida y vuelta con quien está probando.

**2026-09-02, mismo día — conectada a "Programación de Servidores" + historial con contador de no leídas (desplegado, pendiente de que Rafael lo pruebe en su iPhone ya arreglado):**

- **`notificarAsignacionGrupo`** (nueva Cloud Function, `onDocumentWritten('grupos_servidores/{grupoId}')`, región `southamerica-east1` — se autoasignó sola, igual que `actualizarResumenMensual`, por ser un trigger de Firestore) reacciona a CUALQUIER escritura en `grupos_servidores`, sin importar desde qué pantalla/código venga. Compara `fkIdsServidores` de antes/después: al CREAR un grupo, avisa a todos sus integrantes; al EDITARLO, avisa SOLO a los que se acaban de agregar (no a los que ya estaban — evita repetirles el aviso cada vez que se toca el grupo por cualquier otro motivo, ej. cambiarle el nombre). No avisa nada al eliminar un grupo.
- **Historial dentro de la app** (`usuarios/{uid}/notificaciones`, pedido de Rafael: "que aparezcan listadas desde la más nueva arriba" + "el número de notificaciones sin leer"): `enviarNotificacionAUsuarios()` ahora escribe un documento ahí por CADA destinatario, además de mandar el push — funciona aunque esa persona nunca haya activado el permiso del navegador en NINGÚN dispositivo (el historial es independiente del push real). Campos: `titulo`, `cuerpo`, `datos` (ej. `grupoId`/`categoria`), `leida` (bool), `creadoEn`.
- **`widgets/boton_notificaciones.dart`:** al tocar la campana YA ACTIVADA, en vez de solo un `SnackBar` de "ya están activadas", abre una hoja inferior con la lista completa (`orderBy('creadoEn', descending: true)`, límite 50) y las marca todas como leídas al abrirla (`WidgetsBinding.instance.addPostFrameCallback`, para no marcar leído algo que la lista todavía no mostró de verdad si algo falla). La propia campana ahora lleva un `Badge` (widget nativo de Material) con el conteo de no leídas, solo visible cuando hay al menos una — el conteo se deriva del mismo stream de la lista, sin un segundo listener aparte.
- **`firestore.rules`:** `usuarios/{uid}/notificaciones/{notifId}` — `get`/`list` solo el propio dueño; `update` solo el propio dueño Y solo puede tocar el campo `leida` (`request.resource.data.diff(resource.data).affectedKeys().hasOnly(['leida'])`, si intenta cambiar cualquier otra cosa se rechaza); `create`/`delete` siempre `false` desde el cliente — el historial lo escribe únicamente el Admin SDK.
- **✅ Confirmado por Rafael en producción: la notificación llega, la lista se ve ordenada y el contador de no leídas aparece bien.**

**2026-09-02, mismo día — el mensaje ahora incluye la fecha real del próximo servicio, no solo el nombre del grupo:** Rafael pidió que dijera algo como "fuiste asignado para el servicio de tal día con fecha algo así" en vez de solo "fuiste asignado al grupo X de Y". Requirió portar a JavaScript (Cloud Function) el cálculo de rotación que ya existía en Dart (`grupoQueSirve()`/`siguienteGrupoSugerido()`, sección 5.18), pero "al revés": dado UN GRUPO, cuándo le toca — en vez de dada una fecha, qué grupo le toca.

- **`proximaFechaDeGrupo(grupoId, categoriaDoc, gruposDeCategoria)`** (nueva función en `functions/index.js`): si la categoría es `semanal`, calcula cuántas semanas de diferencia hay entre el grupo de referencia (`grupoReferenciaId`) y este grupo según su posición (`orden`), y avanza esa cantidad de semanas desde `fechaReferenciaRotacion` hasta encontrar la primera ocurrencia futura (sumando ciclos completos de "cuántos grupos hay × 7 días" si ya pasó). Si es `manual` (Casa2/Ayunos), busca el `servicios_programados` más próximo con ese `grupoId` — filtro simple por `grupoId` solo, sin necesitar un índice compuesto con `categoria` (el id de un grupo ya es único). `null` en ambos casos si todavía no hay suficiente información configurada (sin punto de partida, o sin ningún servicio programado).
- **`hoyBogotaMedianoche()`/`fechaTextoBogota()`** (nuevas, mismo patrón que el resto del archivo — reutilizan `OFFSET_BOGOTA_HORAS` ya existente): tratan cada `Timestamp` guardado como medianoche Bogotá (`...T05:00:00Z`), el mismo formato que ya usa toda la app desde el bug de `fechaNacimiento` (sección 8) — evita reintroducir ese mismo tipo de corrimiento de día.
- **Mensaje final:** `"Fuiste asignado para el servicio de [categoría] del DD/MM/AAAA."` si se pudo calcular una fecha; si no (categoría semanal sin punto de partida configurado, o manual sin nada programado todavía), cae al mensaje anterior `"Fuiste asignado al grupo '[nombre]' de [categoría]."` — nunca se envía sin avisar nada.
- El cálculo de la fecha va en un `try/catch` aparte del envío en sí — si algo falla calculando la fecha (ej. datos inconsistentes), la notificación se manda igual con el mensaje sin fecha, en vez de no mandarse nada.

**2026-09-02, mismo día — aclaración importante de Rafael sobre CUÁL es el disparador correcto:** tras probar "reasignar" un grupo (sin agregar a nadie nuevo) y no recibir nada, Rafael aclaró que lo que él siempre quiso decir con "asignar un grupo y confirmarlo" es la acción de **"Próximos Servicios"** (elegir/cambiar el punto de partida de la rotación, o programar un servicio de Casa2/Ayunos) — NO editar quién integra un equipo en la pestaña "Grupos". Son dos eventos distintos y ahora los dos generan aviso, cada uno con su propio criterio:

- **`notificarAsignacionGrupo`** (la de antes, sin cambios): reacciona a `grupos_servidores` — solo avisa a quien se ACABA DE AGREGAR al equipo, nunca a quien ya estaba (confirmado explícitamente con Rafael que este es el comportamiento querido para ESTE evento — agregar a alguien a un equipo no es lo mismo que confirmarle que le toca servir).
...
| `admin/programacion_servidores_screen.dart` | **Nuevo 2026-08-31, ampliado 2026-09-02** (ver secciones 5.17/5.18 para el detalle completo) — "Programación de Servidores", 2 pestañas: "Grupos" (crear/editar/eliminar grupos y categorías, asignar servidores e integrantes, reordenar la rotación con ↑/↓) y "Próximos Servicios" (rotación automática calculada para categorías semanales — Domingos, Miércoles — y programación manual con sugerencia de grupo para categorías sin día fijo — Casa2, Ayunos). **Solo administrador y líder de ministerio** (`RolUsuario.puedeGestionarProgramacion` — a propósito sin columna, a diferencia del resto de pantallas de liderazgo). Colecciones `categorias_programacion` + `grupos_servidores` + `servicios_programados`. |
...
- **`image_picker` fallaba en producción** (`MissingPluginException`) pero funcionaba en modo debug → se resolvió con `flutter clean` + `flutter pub get` antes de rebuildear. Si vuelve a pasar algo similar tras agregar un plugin nuevo, probar eso primero. **Volvió a pasar una cuarta vez (2026-09-02) con `firebase_messaging`** (notificaciones push, sección 5.19) — Rafael reportó, ya instalado desde Safari en iPhone, `MissingPluginException: No implementation found for method Messaging#...` al tocar la campana. Mismo fix exacto: `flutter clean` + `flutter pub get` + rebuild + redeploy. **Patrón ya confirmado 4 veces con paquetes nuevos distintos** (`image_picker`, `printing`, `youtube_player_iframe`, `firebase_messaging`) — cuando se agregue OTRO paquete nuevo en el futuro, correr `flutter clean` como parte normal del primer build después de `flutter pub add`, no esperar a que falle en producción para acordarse.
...
- **Vuelta 2, mismo bug (2026-08-30):** Rafael notó que la tarjeta "Presentes ahora" del Dashboard seguía marcando 19 después del fix de arriba. Causa: `admin/dashboard_screen.dart` tenía su PROPIA copia de la lógica de "quién está presente" (`_presentesAhora()`), duplicada de `calcularPresentes()` en vez de reutilizarla — así que el fix de la función compartida nunca la alcanzó. **Corregido eliminando la duplicación por completo**: `_presentesAhora()` se borró y el Dashboard ahora llama directo a `calcularPresentes()`. Se revisó el resto del código buscando más copias del mismo patrón (`ultimoPorNino`/`visitantesPresentes`) — `modo_emergencia_screen.dart` y `cumpleanos_ninos_screen.dart` ya llamaban correctamente a la función compartida, ninguna otra pantalla tenía su propia copia.
...
Nada bloqueante ahora mismo. Última sesión: **2026-09-02**. Lo único abierto:

1. ✅ **Notificaciones de "Programación de Servidores" confirmadas funcionando en producción** (ver sección 5.19) — con las 3 funciones ya desplegadas (`notificarAsignacionGrupo` para roster nuevo, `notificarPuntoDePartidaRotacion` para categorías semanales, `notificarServicioProgramado` para Casa2/Ayunos). **Pendiente:** confirmar en un caso real que al elegir/cambiar el punto de partida en "Próximos Servicios" (o programar un servicio de Casa2/Ayunos), les llega el aviso CON fecha a TODOS los del grupo asignado — esta última parte (las dos funciones nuevas) todavía no se ha probado en producción, solo la de roster (`notificarAsignacionGrupo`).
2. **"Modo emergencia" ya probado y funcionando en computador (2026-08-19)** — ver sección 5.6: activar/desactivar, dar salida con foto+firma, reporte PDF con fotos reales, y el borde rojo de aviso para el admin, todo confirmado por Rafael. **Pendiente:** confirmar en celular (falla ahí mismo el `MissingPluginException` del PDF — mismo patrón resuelto 4 veces con otros paquetes, ver sección 8, pero este caso puntual sigue sin prioridad para Rafael) y confirmar el comportamiento con varias sesiones simultáneas conectadas a la vez (una admin + una servidor normal + una acudiente), que no se probó todavía.
3. **La cámara sigue sin cargar en celular al escanear manilla** — confirmado por Rafael que persiste incluso después del arreglo de raíz (zxing-wasm auto-hospedado). **Decisión explícita de Rafael (2026-08-19): no seguir tocando esto por ahora.** Queda documentado como conocido/pausado, no como resuelto.
4. ✅ **Agregar el acceso directo con Safari en iPhone sí carga bien** — confirmado indirectamente: Rafael instaló RocaKids con Safari en su iPhone (2026-09-02, para probar notificaciones push) y funcionó (ícono en pantalla de inicio, abre en modo standalone, iOS 17).
...
7. **Capturas del manual de usuario cada vez más desactualizadas** (`assets/images/manual/menores_recibidos.png`, `registro_asistencia.png`) — ya venían sin el botón "+"/"Actualizar" desde el 2026-08-24, y desde entonces se agregó además todo "Programación de Servidores" y la campana de notificaciones, ninguna de las dos aparece en el manual todavía. Rafael confirmó explícitamente (2026-08-24) dejarlo pendiente sin prioridad por ahora — sigue sin prioridad, solo señalando que la brecha creció.
8. **Módulo de "Programación de Servidores" — falta la etapa 3 (no pedida todavía):** hoy el sistema calcula/sugiere solo QUIÉN sirve en la próxima ocasión de cada categoría (sección 5.18), pero no hay ninguna pantalla que registre/programe turnos concretos más allá del "próximo" de cada categoría (ej. un calendario completo, o historial de quién sirvió cuándo). Ver [[feature-programacion-servidores]] — pendiente de que Rafael la pida y la defina.
...
**Notas de contexto que ya no son pendientes activas, solo por si hace falta el dato:** "Cumpleaños Servidores" (ítem 4 de versiones anteriores de esta lista) ya no está vacío — para cuando se escribió esto varios servidores reales ya tenían su fecha de nacimiento cargada; y el correo de cumpleaños (ítem 8 de versiones anteriores) no ha vuelto a reportarse tarde desde que se corrigió además el bug de fecha de nacimiento corrida por la migración (2026-08-27, ver sección 8) — ninguno de los dos tuvo una confirmación explícita puntual de Rafael, pero tampoco un reporte de que sigan fallando.
