...
### ✅ Resuelto 2026-08-18: Migración de datos reales (niños + acudientes + relaciones)

Primera tanda del Módulo 2 — migrados **460 niños**, **456 acudientes** (con cuenta de Firebase Auth real, contraseña = su número de documento) y **626 relaciones niño↔acudiente** desde `DB RocaKids V2 (15).xlsx` a producción. Detalle completo, decisiones de diseño y cómo restaurar si algo sale mal: ver [[feature-migracion-modulo2-datos-reales]] en la memoria del proyecto — vale la pena leerla completa antes de tocar esto de nuevo, cubre: por qué 118 niños y 20 acudientes quedaron fuera de esta tanda (sin vínculo familiar real), el manejo de cuentas de Firebase Auth "fantasma" de corridas interrumpidas, y la limitación de Google Drive que dejó pendientes las fotos de la mayoría de acudientes.
...
- **Pendiente real:** ~430 fotos de acudientes (y algunas de niños) quedaron sin subir — Google Drive empezó a bloquear las descargas automáticas por "muchos accesos" a mitad de la migración. Falta que Rafael descargue las carpetas de Drive como ZIP (desde su propio navegador, sin ese límite) para poder terminar de subirlas.
...
4. **Módulo 2 — Migración de datos reales:** ✅ niños/acudientes/relaciones hechos el 2026-08-18 (ver sección "Qué falta" arriba). El archivo real es `D:\Downloads\DB RocaKids V2 (*).xlsx` (⚠️ ojo: NO es la carpeta Descargas normal de Windows, es una ruta directa en el disco D:). **Falta todavía:** completar las fotos pendientes (bloqueadas por Drive), migrar `SERVIDORES` (30 filas — decisión ya tomada con Rafael: rol `pendiente` hasta que un admin lo asigne, contraseña = documento, ver [[feature-migracion-modulo2-datos-reales]]), y los **3873 registros históricos de asistencia** (`REGISTROS` — decisión ya tomada: todos estaban como "Salida" por diseño de la base vieja, se importan como "Entrada" porque una salida implica que hubo ingreso al servicio).
