...
**Explícitamente NO migrado todavía (pedido directo de Rafael, 2026-08-18):** los **3873 registros históricos de asistencia** (`REGISTROS`). Decisión ya tomada para cuando se retome: cada "Salida" histórica se importa como "Entrada" (la base vieja solo registraba salidas, y una salida implica que hubo ingreso al servicio).

**Nuevo, relacionado con la migración:**
- `Nino.estadoRegistro` ahora puede ser `'Graduado'` (más de 10 años) además de `'Activo'` — 51 niños quedaron así en esta tanda. Dashboard: nuevo dato "Niños graduados" y alerta mensual de quién cumple 11 años ese mes (`AuthService.obtenerNinosQueGraduanEsteMes()`), ver `admin/dashboard_screen.dart`. **No hay filtro automático todavía** que oculte a los graduados de la búsqueda en "Registro de asistencia" — sigue pendiente si Rafael lo pide.
- `Acudiente.correoPendienteDeCorregir` (nuevo campo): `true` cuando el acudiente no tiene correo real o llegó duplicado con otro al migrar. Dispara un aviso amarillo en su ficha (`acudiente_detalle_sheet.dart`) y en el check-in (`registro_asistencia_screen.dart`) hasta que alguien le guarde un correo real desde "Editar información" (`AuthService.editarAcudiente()` lo apaga solo).
- **Bloque "Pendientes" en el Dashboard** (interactivo): tarjetas tocables — niños sin foto, niños sin documento, acudientes sin foto, acudientes con correo pendiente — que al tocarlas abren la lista exacta de quiénes son, y tocar a alguien de la lista abre su ficha completa para corregirlo ahí mismo. Ver `admin/dashboard_screen.dart`.
- **Fotos: ✅ completas** (2026-08-18, misma tarde) — Rafael descargó las dos carpetas de Drive como ZIP desde su propio navegador (evita el límite de accesos que bloqueó la descarga automática) y se subieron todas: 432 niños y la mayoría de los 456 acudientes migrados quedaron con foto (algunos ya tenían la suya propia de antes, no se sobreescribió). Detalle técnico en [[feature-migracion-modulo2-datos-reales]].

---

### ✅ Resuelto 2026-08-18: Dashboard con gráficas

Implementado y desplegado — ver `admin/dashboard_screen.dart` en la tabla de pantallas (sección 6) para el detalle completo. Gate de rol confirmado como `RolUsuario.puedeVerDashboard` (administrador, columna, líder de ministerio — mismo conjunto que `puedeVerAcudientesYNinos`). Verificación visual pendiente de que Rafael la confirme en producción (esta sesión no pudo tomar screenshot del Browser pane — limitación ya documentada, ver [[dev-server-testing]]); `flutter analyze` sin errores y build/deploy exitosos.

**Pendiente real que quedó fuera de esta primera versión:** si el volumen de `registros` crece mucho (ej. tras importar los ~3873 registros históricos del Módulo 2, punto 4 más abajo), la consulta directa del bloque "Histórico" puede volverse lenta/costosa — resolver entonces con una tabla de resúmenes pre-calculados (mismo patrón que el cierre automático de sección 5.5), no antes.
...
4. **Módulo 2 — Migración de datos reales:** ✅ niños/acudientes/relaciones/servidores hechos el 2026-08-18 (ver sección "Qué falta" arriba). El archivo real es `D:\Downloads\DB RocaKids V2 (*).xlsx` (⚠️ ojo: NO es la carpeta Descargas normal de Windows, es una ruta directa en el disco D:). **Falta todavía:** completar las fotos pendientes (bloqueadas por Drive, Rafael va a compartir un ZIP) y los **3873 registros históricos de asistencia** (`REGISTROS` — explícitamente pospuesto a pedido de Rafael 2026-08-18, "NO cargues aún ningún registro histórico"; decisión de transformación ya tomada para cuando se retome, ver [[feature-migracion-modulo2-datos-reales]]).
