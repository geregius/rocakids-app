...
### ⚠️ Acción pendiente inmediata (2026-08-17)

Correr una vez el botón admin **"Sincronizar presencia de niños"** (menú de `AppShell`) — marca `presente: true` en los niños que ya estaban adentro ANTES del bloqueo de Entrada duplicada (ver sección 5). Sin esto, esos niños específicos podrían quedar sin el flag hasta su próximo movimiento (no es peligroso — a lo sumo permite una entrada de más una sola vez — pero es mejor correrlo).
