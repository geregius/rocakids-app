...
| `ninos_presentes_screen.dart` | Sección "Niños presentes hoy" (nuevo 2026-08-15, todos los roles principales) — quiénes están AHORA MISMO en el salón (último movimiento de hoy = "Entrada"), subdivididos por grupo de edad, cada grupo con su total. Los visitantes cuentan también (cada Entrada de hoy, ya que esta fase no tiene forma de registrarles una Salida — limitación conocida, no bug). Usa `AuthService.registrosDeHoy()` (stream con rango de fecha del día) + `obtenerTodosLosNinos()` (una sola lectura, para cruzar nombre/foto/alerta médica). **Pendiente explícito, para después:** una vista histórica del día completo que incluya a quien ya salió (decisión de Rafael de dejarla para otra sesión). |
