...
| `admin/dashboard_screen.dart` | Sección "Dashboard" (nuevo 2026-08-18, **solo administrador, columna y líder de ministerio** — `RolUsuario.puedeVerDashboard`, mismo gate que "Acudientes y Niños") — gráficas de barras con `fl_chart`. **Bloque "Hoy"** (reactivo, `registrosDeHoy()`): tarjetas de total recibidos/presentes ahora/ya salieron/visitantes/sin documento, y dos gráficas (por grupo de edad, por servicio) — reutiliza la misma lógica de "presentes ahora" que `ninos_presentes_screen.dart`. **Bloque "Histórico"** (consulta directa de una sola vez, no reactiva, `AuthService.obtenerEntradasDesde()`, con selector de 4/8/12 semanas): entradas por semana y comparación entre servicios, más el total actual de niños en el sistema (`contarNinosRegistrados()`, un dato simple — NO una tendencia, porque los documentos de `ninos` no guardan fecha de creación, así que no hay forma de reconstruir ese histórico). Requirió un índice compuesto nuevo en `firestore.indexes.json` (`tipoMovimiento` + `fechaMovimiento`) para la consulta histórica. Decisión explícita: empezar con consultas directas y resolver con una tabla de resúmenes pre-calculados (mismo patrón que el cierre automático, sección 5.5) **solo si se vuelve lento** con más volumen — no antes. |
